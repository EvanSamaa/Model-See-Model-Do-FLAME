...
def add_data_options(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--dataset_type', type=str, default='HDTF_TFHP', help='this can either be HDTF_TFHP or celebv-text-toy or celebv-text')
...
def add_training_options(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--l_kl_div', type=float, default=1E-5,
                    help='weight of the head constraint during window transition')
...
def add_additional_options(parser: argparse.ArgumentParser, pre_args=None):
    if pre_args is not None:
        args = parser.parse_known_args(pre_args)[0]
    else:
